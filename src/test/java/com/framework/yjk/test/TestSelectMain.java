...
public class TestSelectMain {
    ...
    public void test3() throws ClassNotFoundException, SQLException{
       ...
       String query = "SELECT REF_ID AS referId ,B_QTY AS buyQty,B_FULL_SUM AS buyFullSum,B_PROFIT AS buyProfit,B_ORDERNO AS buyOrderNo,B_TRADENO AS buyTradeNo,B_PRICE AS buyPrice,S_QTY AS sellQty,S_TRADENO  AS sellTradeNo,S_PROFIT AS sellProfit,S_PRICE  AS sellPrice,S_ORDERNO AS sellOrderNo,S_FULL_SUM AS sellFullSum,FBACK_TYPE AS feedBackType,D_ORDERNO AS doubleQuoteOrderNo,OB_ACC,ORDER_DATE,TRADER,ORDER_TIME,OB_COMPANY,REMARK,REC_DATE,SETTLE,STATUS,OB_SETTLE,INTEREST,ACC,TRADE_TIME,NICKED,STOCK_NAME,REC_NUM,STOCK_CODE,COMPANY,REC_TIME,ICE_QTY,FIRM,DIR,WITHDRAW,PRODUCT,TRADE_DATE,OB_FIRM,OB_TRADER,TRADE_TYPE, DEAL_FLAG, RETRY_NUM, DEAL_TIME,ERROR_MSG " +
               "FROM zq_feedback WHERE REC_DATE='20180920' AND  DEAL_FLAG !='Y' AND FBACK_TYPE != 'd' AND  RETRY_NUM < 5";
        //String query = "select * from zq_feedback where  REC_NUM > 10  and  REC_NUM<10000000  and  REC_DATE='20180912'";
        DbfStatement statement = (DbfStatement) connection.prepareStatement(query);
        /*String query = "SELECT rec_num as A  FROM zq_order where rec_num = 2000000 and  STOCK_CODE = '500500' LIMIT 1";*/
